    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                ...
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4 flex-1">
                            ...
                            <p class="mt-1 text-2xl font-bold text-slate-900">Rp {{ number_format($todaySales->total_revenue ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-slate-50 px-5 py-3 text-sm text-slate-500">Per {{ now()->format('d M Y') }}</div>
            </div>
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-900/5">
                ...
                <div class="bg-slate-50 px-5 py-3 text-sm text-slate-500">Transaksi selesai hari ini</div>
            </div>
                ...
                <div class="p-5">
                    <div class="flex items-center">
                        <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-100">
                            <svg class="h-6 w-6 text-cyan-700" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                            </svg>
                        </div>
                        <div class="ml-4 flex-1">
                            <p class="text-sm font-medium text-slate-500">Pembayaran Hari Ini</p>
                            <p class="mt-1 text-2xl font-bold text-slate-900">Rp {{ number_format($todayPaymentTotal, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-slate-50 px-5 py-3 text-sm text-slate-500">{{ $todayPayments->count() }} metode pembayaran</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-900/5">
                ...
                <div class="bg-slate-50 px-5 py-3">
                    @if($lowStockProducts > 1)
                        <details class="group">
                            <summary class="flex cursor-pointer list-none items-center justify-between text-sm font-medium text-amber-700">
                            ...
                            <div class="mt-3 max-h-72 overflow-auto rounded-lg border border-amber-100 bg-white">
                                <table class="min-w-full divide-y divide-amber-100">
                                    <thead class="bg-amber-50">
                                        <tr>
                                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-amber-700">Nama Obat</th>
                                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-amber-700">No Batch</th>
                                            <th class="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wide text-amber-700">Stok</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-amber-50">
                                        @foreach($lowStockItems as $item)
                                            <tr>
                                                <td class="px-3 py-2 text-sm text-slate-700">{{ $item->product?->name ?? '-' }}</td>
                                                <td class="px-3 py-2 text-sm text-slate-600">{{ $item->batch_number }}</td>
                                                <td class="px-3 py-2 text-right text-sm font-semibold text-slate-800">{{ $item->stock }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                    ...
                    @else
                        <span class="text-sm text-slate-500">Ambang stok menipis: &lt; {{ $lowStockThreshold }}</span>
                    @endif
                </div>
            </div>
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-900/5">
                ...
                <div class="bg-slate-50 px-5 py-3">
                    @if($expiringBatches > 1)
                        <details class="group">
                            <summary class="flex cursor-pointer list-none items-center justify-between text-sm font-medium text-red-700">
                                <span>Lihat detail segera expired</span>
                                <svg class="h-4 w-4 transition-transform group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                                </svg>
                            </summary>
                            <div class="mt-3 max-h-72 overflow-auto rounded-lg border border-red-100 bg-white">
                                <table class="min-w-full divide-y divide-red-100">
                                    <thead class="bg-red-50">
                                        <tr>
                                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-red-700">Nama Obat</th>
                                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-red-700">No Batch</th>
                                            <th class="px-3 py-2 text-right text-xs font-semibold uppercase tracking-wide text-red-700">Stok</th>
                                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-red-700">Tanggal Expired</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-red-50">
                                        @foreach($expiringBatchItems as $item)
                                            <tr>
                                                <td class="px-3 py-2 text-sm text-slate-700">{{ $item->product?->name ?? '-' }}</td>
                                                <td class="px-3 py-2 text-sm text-slate-600">{{ $item->batch_number }}</td>
                                                <td class="px-3 py-2 text-right text-sm font-semibold text-slate-800">{{ $item->stock }}</td>
                                                <td class="px-3 py-2 text-sm text-slate-600">{{ $item->expired_date?->format('d M Y') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
        ...
        <div>
            ...
            <div class="rounded-xl bg-white shadow-sm ring-1 ring-slate-900/5 p-6">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr>
                                <th class="py-2 text-left text-xs font-medium uppercase text-slate-500">Invoice</th>
                                <th class="py-2 text-left text-xs font-medium uppercase text-slate-500">Waktu</th>
                                <th class="py-2 text-left text-xs font-medium uppercase text-slate-500">Pembayaran</th>
                                <th class="py-2 text-right text-xs font-medium uppercase text-slate-500">Total</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach($recentSales as $sale)
                                <tr>
                                    ...
                                    <td class="py-3 text-right text-sm font-medium text-slate-900">Rp {{ number_format($sale->total, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
            <div class="rounded-xl bg-white p-6 text-sm text-slate-500 shadow-sm ring-1 ring-slate-900/5">
